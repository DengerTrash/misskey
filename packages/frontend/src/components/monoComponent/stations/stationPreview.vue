...
<div style="position: relative;">
	...
	>
		<div class="banner" :style="bannerStyle">
			<div class="fade"></div>
			<div class="name"><i class="ti ti-device-tv"></i> {{ station.name }}</div>
			<div v-if="station.isSensitive" class="sensitiveIndicator">{{ i18n.ts.sensitive }}</div>
			<div class="status">
				<div>
					<i class="ti ti-users ti-fw"></i>
					<I18n :src="i18n.ts._channel.usersCount" tag="span" style="margin-left: 4px;">
						<template #n>
							<b>{{ station.usersCount }}</b>
						</template>
					</I18n>
				</div>
				<div>
					...
					<I18n :src="i18n.ts._channel.notesCount" tag="span" style="margin-left: 4px;">
						<template #n>
							<b>{{ station.notesCount }}</b>
						</template>
					</I18n>
				</div>
				<div v-if="$i != null && $i.id === station.userId" style="color: var(--MI_THEME-warn)">
					<i class="ti ti-user-star ti-fw"></i>
					<span style="margin-left: 4px;">{{ i18n.ts.youAreAdmin }}</span>
				</div>
			</div>
		</div>
		<article v-if="station.description">
			<p :title="station.description">{{ station.description.length > 85 ? channel.description.slice(0, 85) + '…' : channel.description }}</p>
		</article>
		<footer>
			<span v-if="station.lastNotedAt">
				{{ i18n.ts.updatedAt }}: <MkTime :time="station.lastNotedAt"/>
			</span>
		</footer>
	</MkA>
	<div
...
const props = defineProps<{
	station: Misskey.entities.Channel;
}>();
